...
def create_instance(compute, project, zone, name, bucket, output_folder):
...
def main(project, bucket, zone, instance_name, output_folder):
    compute = googleapiclient.discovery.build('compute', 'v1')
    try:

        print('Creating instance.')

        operation = create_instance(compute, project, zone, instance_name, bucket, output_folder)
...
def run():
    args = parse_args()
    main(args.project_id, args.bucket_name, args.zone, args.name, args.output_folder)
...
def parse_args():
    ...
    parser.add_argument('git_project', type=str, help='Your git project to execute.')
